...
class Laporan_alumni extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('Laporan_alumni_model', 'ion_auth');
    }
    ...
    public function index()
    {
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('lulus_laporan/laporan_alumni/_css');
            $this->load->view('layout/header-bottom', $data);
            $this->load->view('layout/main-navigation', $data);
            $this->load->view('lulus_laporan/laporan_alumni/_v', $data);
            $this->load->view('layout/footer-top');
            $this->load->view('lulus_laporan/laporan_alumni/_js');
    ...
    public function tabel($id, $tgl)
    {
        ...
        if ($realDiff < 32) {            
            ...
            $html = '  <table class="table datatable" style="font-size: 14px">
                            ...
                            <tbody>';
                                if (!empty($siswa)) {
                                    $no = 1;
                                    foreach ($siswa as $s) {                                    
                                        $html .= '<td>' . $no++ . '</td><td>' . $s['nama_siswa'] . '</td><td>' . $s['nis'] . '</td><td>' . $s['tahun_pelajaran'] . '</td><td>' . $s['telp'] . '</td><td>' . $s['email'] . '</td></tr>';
                                    }
                                }';                       
                            </tbody>
                        </table>
            ...
            $this->d['html'] = $html;
            $this->load->view('lulus_laporan/laporan_alumni/_list', $this->d);
        } else {
