...
<script type="text/javascript">
    ...
    $('.btn-edit').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>/biaya/" + id,
            data: "jenis=edit",
    ...
    $('.btn-periksa').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>/periksa/" + id,
            data: "jenis=periksa",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-periksa').modal({
    ...
    $('.btn-detail').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>/detail/" + id,
            data: "jenis=detail",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-detail').modal({
