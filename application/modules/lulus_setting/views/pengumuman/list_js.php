...
<script type="text/javascript">
    ...
    $('.btn-action').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>/tambah_pengumuman",
            data: "jenis=tambah",
    ...
    $('.btn-edit').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>/edit_pengumuman/" + id,
            data: "jenis=edit",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-edit').modal({
